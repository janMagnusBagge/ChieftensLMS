...
namespace ChieftensLMS.Controllers
{
	...
	public class ManageController : Controller
	{
		...
		public async Task<ActionResult> Index(ManageMessageId? message, string id)
		{
			...
			IList<IdentityRole> roles = _studentAccountService.GetAllRoles().ToList();

			IEnumerable<SelectListItem> rolesSelectItems = roles.Select(r => new SelectListItem()
			{
				Selected = UserManager.IsInRole(userId, _studentAccountService.GetAllRoles().FirstOrDefault(e => e.Id == r.Id).Name),
				Text = r.Name,
				Value = r.Name
			});
